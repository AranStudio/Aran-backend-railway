...
function toClientError(err, fallback = "Visual request failed") {
  ...
  try {
    ...
    const frames = await mapWithLimit(beats, 2, async (beat, i) => {
      const imgPrompt = [
        ...
        `Strong composition, tasteful color, believable camera feel.`,
        `NO text, NO typography, NO logos, NO watermarks.`,
        `Wide-cinema framing vibe even if square crop.`,
        ``,
        `Story: ${prompt}`,
        `Type: ${contentType || ""}`,
        `Beat ${i + 1}: ${beat}`,
    ...
    const { status, msg } = toClientError(err, "Visuals failed");
